...
def ingest_data_bulk(data):
    ...
    try:
        ...
        if to_delete:
            ...
            cur.execute("DELETE FROM parameter WHERE vector_id = ANY(%s::uuid[])", (list(to_delete),))
            cur.execute("DELETE FROM response WHERE vector_id = ANY(%s::uuid[])", (list(to_delete),))
            # Then delete main rows
            cur.execute("DELETE FROM intent WHERE vector_id = ANY(%s::uuid[])", (list(to_delete),))
            cur.execute("DELETE FROM intent_embedding WHERE id = ANY(%s::uuid[])", (list(to_delete),))
